...
- SheetsAppend: config.spreadsheet_id (required — the sheet's human-readable name, e.g. "Inbox Log", is resolved to a real ID automatically; a real ID also works), config.row (optional object), config.range (optional, default "A1")
- SheetsReadRows: config.spreadsheet_id (required — same name-or-ID resolution as SheetsAppend), config.range (optional, default "Sheet1")
